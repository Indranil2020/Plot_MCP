        ...
        if format_type == "csv":
            ...
            for delim in delimiters:
                try:
                    ...
                    if len(lines) > 0:
                        first_line_cols = len(lines[0].split(delim))
                        # Check if this delimiter gives consistent column count
                        consistent = all(len(line.split(delim)) == first_line_cols for line in lines[:5] if line.strip())
